...
int main()
{
    ...
    double sigma2=0.7499;
    for (unsigned index = 0; index < 1; index++)
    {
        //sigma2+=0.1;
        unsigned quansize = 16;
        std::vector<std::vector<double>> prob_join_xy = gaussian_disretization(-10,10,2000, sigma2);
        std::cout<<prob_join_xy[0][0]<<"  "<<prob_join_xy[1][0]<<std::endl;
        IB_kernel kernel_instance(prob_join_xy, quansize, 2000);
    ...
    std::vector<double> vari_edge_dist{0,0.2558,0.3148,0.0465,0,0,0,0,0,0,0.3837};
    ...
    bool flag=true;
    int regular_result;
    double llr_intervel=pow(10.0,-3.0);
    unsigned int ib_runtime=2000;
    while (flag)
    {
