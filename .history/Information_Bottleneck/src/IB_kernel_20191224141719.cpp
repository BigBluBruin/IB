...
void IB_kernel::smIB2(std::vector<double> ref_llr)
{
    ...
    //std::cout << "best dis" << best_dis << std::endl;
    ...
    //-------------------------------------------------------------------
    cluster=all_partiton[best_pos];
    //std::cout<<"best pos: "<<best_pos<<" mi diff"<<all_mi[pos[0]]-all_mi[best_pos] <<std::endl;
    prob_join_xt=quantize_to_xt(prob_join_xy,cluster);
    mi=it_mi(prob_join_xt);
    unsigned counter;
    for(unsigned index=1;index<=quan_size/2-1;index++)
...
std::vector<std::vector<double>> IB_kernel::find_threshold(unsigned left_most, unsigned right_most)
{
    ...
    std::vector<std::vector<double>> right_join_prob(2), left_join_prob(2), single_join_prob(2);
    ...
    for (unsigned index = 0; index < total_time; index++)
    {
        ...
        left_join_prob[0].clear();
        left_join_prob[0].push_back(std::accumulate(prob_join_xy[0].begin() + left_most, prob_join_xy[0].begin() + single_point_pos,0.0));
        left_join_prob[1].clear();
        left_join_prob[1].push_back(std::accumulate(prob_join_xy[1].begin() + left_most, prob_join_xy[1].begin() + single_point_pos,0.0));
        std::copy(prob_join_xy[1].begin() + left_most, prob_join_xy[1].begin() + single_point_pos, std::back_inserter(left_join_prob[1]));
        right_join_prob[0].clear();
        std::copy(prob_join_xy[0].begin() + single_point_pos + 1, prob_join_xy[0].begin() + right_most, std::back_inserter(right_join_prob[0]));
        right_join_prob[1].clear();
        std::copy(prob_join_xy[1].begin() + single_point_pos + 1, prob_join_xy[1].begin() + right_most, std::back_inserter(right_join_prob[1]));
        single_join_prob[0].clear();
        single_join_prob[0].push_back(prob_join_xy[0][single_point_pos]);
        single_join_prob[1].clear();
        single_join_prob[1].push_back(prob_join_xy[0][single_point_pos]);
    }
