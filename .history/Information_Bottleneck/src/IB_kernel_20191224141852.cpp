...
std::vector<std::vector<double>> IB_kernel::find_threshold(unsigned left_most, unsigned right_most)
{
    ...
    for (unsigned index = 0; index < total_time; index++)
    {
        ...
        right_join_prob[0].push_back(std::accumulate(prob_join_xy[0].begin() + single_point_pos + 1, prob_join_xy[0].begin() + right_most,0.0));
        right_join_prob[1].clear();
        right_join_prob[1].push_back(std::accumulate(prob_join_xy[1].begin() + single_point_pos + 1, prob_join_xy[1].begin() + right_most,0.0));
        std::copy( std::back_inserter(right_join_prob[1]));
