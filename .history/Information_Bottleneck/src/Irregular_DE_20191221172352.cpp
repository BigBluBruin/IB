...
{
    std::vector<std::vector<double>> combined_check_dist(2), combined_vari_dist(2),llr_combined_check_dist, llr_combined_vari_dist;
    ...
    for (unsigned iter = 0; iter < max_iter; iter++)
    {
        ...
        for (unsigned ii = 0; ii < check_edge_dist.size() - 2; ii++)
        {
            ...
            combined_output = prob_combination_v2(first_input, second_input, "check",pow(10,-7));
            prob_sort(combined_output);
            prob_llr_combined = llr_combination(combined_output, llr_combination_interval);
        ...
        llr_file.close();
        IB_kernel check_IB(llr_combined_check_dist, quantization_size, ib_runtime);
        check_IB.smIB();
        ...
        for (unsigned ii = 0; ii < vari_edge_dist.size() - 1; ii++)
        {
            ...
            combined_output = prob_combination_v2(first_input, second_input, "vari",pow(10,-7));
            prob_sort(combined_output);
            prob_llr_combined = llr_combination(combined_output, llr_combination_interval);
            ave_joinprob_llr(prob_llr_combined, pow(10.0, -80.0));
            first_input = prob_llr_combined;
        ...
        ave_joinprob_llr(llr_combined_vari_dist, pow(10.0, -80.0));
        ...
        llr_file.close();
        IB_kernel vari_IB(llr_combined_vari_dist, quantization_size, ib_runtime);
        vari_IB.smIB();
