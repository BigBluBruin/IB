...
bool QCDE(const std::string filename, unsigned quansize)
{
    ...
    if (handle_file.is_open())
    {
        ...
        std::cout<<total_points<<"  "<<max_iter;
        ...
        for (unsigned cur_iter = 0; cur_iter < max_iter; cur_iter++)
        {
            ...
            for (unsigned ii = 0; ii < total_points / 2; ii++)
            {
                //std::cout<<vari_llr[cur_iter][ii]<<"  "<<std::endl;
            ...
            vari_threshold.push_back(this_vari_ib.threshold);
            vari_recons.push_back(llr_cal(this_vari_ib.prob_join_xt));
            std::cout << "finished vari iter " << cur_iter << std::endl;
